...
import type { ServiceTagFormValue } from "@/features/services/lib/service-tags";
import { Plus, Trash2 } from "lucide-react";
import { useTranslation } from "react-i18next";

const EMPTY_TAG: ServiceTagFormValue = { name: "", index: true, follow: true };

type ServiceTagsFieldProps = {
  value: ServiceTagFormValue[];
  onChange: (tags: ServiceTagFormValue[]) => void;
};

export function ServiceTagsField({ value, onChange }: ServiceTagsFieldProps) {
  const { t } = useTranslation("translation", { keyPrefix: "services.form" });
  const tags = value.length ? value : [EMPTY_TAG];

  const updateAt = (index: number, patch: Partial<ServiceTagFormValue>) => {
  ...
  return (
    <Field className="space-y-3 border-t pt-8">
      <div>
