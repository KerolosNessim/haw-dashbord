  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-bold">{t("title")} (AI)</h1>
        <div className="flex flex-wrap items-center gap-2">
          <Can permission="services.view">
            <Button
              ...
              onClick={() => exportBulk([...selectedIds])}
            >
              {isExporting ? (
          ...
          <Can permission="services.create">
            <Button asChild size="lg" className="rounded-xl">
              <Link to="/ai-services/create" className="flex items-center gap-2">
                <Plus />
                {t("add_service")} (AI)
              </Link>
            </Button>
      ...
      {services.length > 0 ? (
        <div className="flex flex-wrap items-center gap-3 rounded-xl border bg-muted/30 px-4 py-3">
          ...
          <span className="text-sm text-muted-foreground">{t("import_export_hint")}</span>
        </div>
      ) : null}
