import ServiceCatalogTable from "@/features/service-catalog/components/service-catalog-table";
...
export default function ServiceCatalogPage() {
  const { t } = useTranslation("translation", { keyPrefix: "service_catalog" });
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-8 pb-20">
      <div className="flex flex-col justify-between gap-6 border-b pb-8 md:flex-row md:items-center">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-muted-foreground">{t("description")}</p>
          </div>
        </div>

        <Can permission="service-catalog.create">
          <Button
            size="lg"
      ...
      {catalogPerms.view ? <ServiceCatalogTable /> : null}
    </div>
  );
}
